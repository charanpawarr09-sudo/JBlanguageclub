...
export async function sendRegistrationConfirmation(params: {
  ...
  try {
    ...
    await resend.emails.send({
      from: FROM_EMAIL,
      to,
      subject: `🎉 Registration Confirmed — ${safeEventTitle} | VOXERA 2026`,
      html: `
        <div style="font-family: 'Inter', system-ui, sans-serif; max-width: 600px; margin: 0 auto; background: #0F172A; color: #F8FAFC; padding: 40px 30px; border-radius: 16px;">
          <h1 style="font-size: 28px; margin-bottom: 8px; color: #fff;">Hey ${safeName}! 🎉</h1>
          <p style="color: #94A3B8; font-size: 16px; margin-bottom: 30px;">Your registration for <strong style="color: #A78BFA;">${safeEventTitle}</strong> at VOXERA 2026 has been confirmed.</p>
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <blockquote style="border-left: 3px solid #7C3AED; padding-left: 12px; color: #333;">${safeMessage}</blockquote>
        </div>
      `,
    });
