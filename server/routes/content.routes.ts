...
router.post('/api/admin/faqs', verifyAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const [created] = await db.insert(schema.faq).values({ ...stripTimestamps(req.body), created_at: new Date(), updated_at: new Date() } as any).returning();
...
router.put('/api/admin/faqs/:id', verifyAdmin, async (req: AuthRequest, res: Response) => {
    try {
        ...
        const [updated] = await db.update(schema.faq).set({ ...stripTimestamps(req.body), updated_at: new Date() } as any).where(eq(schema.faq.id, id)).returning();
...
router.post('/api/admin/testimonials', verifyAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const [created] = await db.insert(schema.testimonials).values({ ...stripTimestamps(req.body), created_at: new Date(), updated_at: new Date() } as any).returning();
...
router.put('/api/admin/testimonials/:id', verifyAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const id = parseInt(req.params.id);
        const [updated] = await db.update(schema.testimonials).set({ ...stripTimestamps(req.body), updated_at: new Date() } as any).where(eq(schema.testimonials.id, id)).returning();
...
router.post('/api/admin/timeline', verifyAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const [created] = await db.insert(schema.timelineEvents).values({ ...stripTimestamps(req.body), created_at: new Date(), updated_at: new Date() } as any).returning();
...
router.put('/api/admin/timeline/:id', verifyAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const id = parseInt(req.params.id);
        const [updated] = await db.update(schema.timelineEvents).set({ ...stripTimestamps(req.body), updated_at: new Date() } as any).where(eq(schema.timelineEvents.id, id)).returning();
...
router.post('/api/admin/announcements', verifyAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const [created] = await db.insert(schema.announcements).values({ ...stripTimestamps(req.body), created_at: new Date(), updated_at: new Date() } as any).returning();
...
router.put('/api/admin/announcements/:id', verifyAdmin, async (req: AuthRequest, res: Response) => {
    try {
        const id = parseInt(req.params.id);
        const [updated] = await db.update(schema.announcements).set({ ...stripTimestamps(req.body), updated_at: new Date() } as any).where(eq(schema.announcements.id, id)).returning();
